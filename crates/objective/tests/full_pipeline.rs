...
async fn test_full_pipeline_ingestion_to_extraction_to_api() {
    ...
    let adapter = RssSourceAdapter::from_xml(
        "integration_test",
    ...
    assert!(documents
        .iter()
        .any(|d| d.title.as_deref() == Some("Tesla announces new Gigafactory in Nevada")));
    assert!(documents
        .iter()
        .any(|d| d.title.as_deref() == Some("Apple expands Austin manufacturing")));
    ...
    let tesla_extraction = extractions
        .iter()
        .find(|e| {
            e.document_id != extractions[0].document_id
                || e.entities.iter().any(|ent| ent.name == "Tesla Inc")
        })
        .unwrap();

    assert!(tesla_extraction
        .entities
        .iter()
        .any(|e| e.name == "Tesla Inc"));
    assert!(tesla_extraction
        .claims
        .iter()
        .any(|c| c.subject_name == "Tesla Inc"));
    ...
    assert!(events
        .iter()
        .all(|e| e.1.event_type == "ingestion.document.received"));
}
...
async fn test_ingestion_deduplicates_by_content_hash() {
    ...
    assert_eq!(
        documents.len(),
        2,
        "should still have only 2 documents after dedup"
    );
}
...
async fn test_extraction_produces_claims_with_confidence() {
    ...
    for document in store.list_documents().await.unwrap() {
        ...
        for claim in &extraction.claims {
            ...
            assert!(
                !claim.evidence_snippet.is_empty(),
                "claim should have evidence"
            );
        }
